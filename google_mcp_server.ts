...
class GoogleServicesMCPServer {
  ...
  private async createSpreadsheet(args: any) {
    const resource = {
      ...
      sheets: args.sheetNames ? args.sheetNames.map(name => ({
  ...
  private async writeDocument(args: any) {
    const requests = [{
      insertText: {
        location: {
          index: args.index || 1, // Insert at beginning if no index specified
        },
        text: args.text,
      },
    }];
    ...
    switch (args.questionType) {
      case 'MULTIPLE_CHOICE':
        questionItem.questionItem.question.choiceQuestion = {
          type: 'RADIO',
          options: args.options?.map(option => ({ value: option })) || [],
        };
        break;
      case 'CHECKBOX':
        questionItem.questionItem.question.choiceQuestion = {
